...
class Room extends CI_Controller
{
    ...
    {
        $data["room_type_array"]=$this->room_model->getRoomArray();
    ...
    {
        $d["total_list"]=$this->room_model->getRommTypeCountAndPrice(isset($nearHours)?getNearTimestampString($nearHours):null);
        $hourStr=$nearHours>=24?($nearHours/24)."天":$nearHours."小时";
        $hourStr=isset($nearHours)?$hourStr:"";
        $d["title"]="自由场<b>".$hourStr."</b>概览--总共开过<b>".$d["total_list"]["total"]."</b>场";
        ...
        foreach ($d['room_type_array'] as $rk => $rn)
        {
            ...
            $infoStr.= "<span style='min-width: 150px;line-height:32px;float: left;display: block'>".$rn.":";
